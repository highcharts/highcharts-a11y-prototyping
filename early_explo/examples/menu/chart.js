...
let patternNodes = []
let patternData = [...sankeyData]
sankeyNodes.forEach(n => {
    const rgb = hexToRgb(n.color)
    const patternUrl = `url(#highcharts-pattern-${rgb.r}-${rgb.g}-${rgb.b})`
    let newNode = {...n}
...
let tooltipLabel = chart.tooltip.getLabel()
tooltipLabel.hide()
shadowChart = {
    title: {...chart.title.styles},
    subtitle: {...chart.subtitle.styles},
    series: {...chart.series[0].options.dataLabels.style},
    tooltip: {...tooltipLabel.text.styles}
}